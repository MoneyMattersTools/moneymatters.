...
## 41. Recurring Bug â€” Dashboard Overlap/Crowding (locked 2026-07-30, HIGH PRIORITY)
...
## 42. Round 28 â€” Free-Plus Incentive Update + Plus Member Logo (locked 2026-07-30)
...
## 44. Section 41 Follow-up â€” Dynamic Figure Sizing (locked 2026-07-30)
...
## 45. Round 29 Feedback (locked 2026-07-30)

1. **Logo revert**: use the two real assets now in moneymatters/images/ (moneymatters-hero-icon.png for the standard/default logo, moneymatters-plus-mark.png for the Plus-member variant) - replace the auto-generated placeholder SVG and any unintended sitewide logo replacement with these.
2. **Footer imagery**: replace the current simple line-art dollar-tree graphic with something more detailed and photographic - matching the vibrant, cinematic imagery style already used in page headers elsewhere on the site, not a flat icon.
3. **Feedback pop-up placement bug**: the "How useful was this?" star-rating widget is incorrectly appearing on the homepage dashboard (below the score/snapshot), not just after actual tool/quiz submissions as originally specified (Section 26 item 3, Section 39 item 3). Find and fix wherever it is being triggered on the homepage generally - it should only ever fire after a real tool or quiz submission.
4. **Stripe - full Plus-gating consistency**: once the checkout flow itself is confirmed working end to end, verify that every other Plus-gated entry point sitewide (Advanced Tools access prompts, private Discord references, and any other MoneyMatters+ offering location) consistently routes to the same subscribe/checkout flow - not just the main MoneyMatters+ page button.
5. **Free Advisor Review page - font sizing refinement**: increase size further, and appropriately for each distinct text element (heading, subhead, checkbox labels, the "anything else" free-text field) rather than one uniform bump - each should be sized to read clearly on its own.
